...
# [getSignatureStatuses RPC Method](/vi/docs/rpc/http/getsignaturestatuses)
...
[txid](/vi/docs/terminology#transaction-id), the first signature of a
transaction.
...
An array of `RpcResponse<object>` consisting of either:
  ...
  * `<object>`
    ...
    * `confirmationStatus: <string|null>` \- The transaction's cluster confirmation status; Either `processed`, `confirmed`, or `finalized`. See [Commitment](/vi/docs/rpc#configuring-state-commitment) for more on optimistic confirmation.
...
[Previous«
getRecentPrioritizationFees](/vi/docs/rpc/http/getrecentprioritizationfees)[NextgetSignaturesForAddress
»](/vi/docs/rpc/http/getsignaturesforaddress)
