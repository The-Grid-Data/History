[![Hivemapper
Logo](data:image/svg+xml;base64,)](/explorer)

Build the Map

Use the Map

[![Bee Blog
Icon](data:image/svg+xml;base64,)The
Latest Buzz](https://hivemapper.com/blog/)

January 17, 2024

# How to Build a Hivemapper Fleet and Recruit Drivers

![](https://images.ctfassets.net/n5g2b08mtiib/6n10PYgiUTfhyy1Aph9JUt/6587477160d134830d7c975f6296ed41/Hivemapper_-
_How_to_Build_a_Hivemapper_Fleet_and_Recruit_Drivers_-
_v2.jpg?w=750&h=422&fl=progressive&q=100&fm=jpg)![](https://images.ctfassets.net/n5g2b08mtiib/6n10PYgiUTfhyy1Aph9JUt/6587477160d134830d7c975f6296ed41/Hivemapper_-
_How_to_Build_a_Hivemapper_Fleet_and_Recruit_Drivers_-
_v2.jpg?w=750&h=422&fl=progressive&q=100&fm=jpg)

## **An experienced fleet manager shares their time-tested strategies for
creating a Hivemapper fleet.**

Hivemapper is building the worldâs freshest map with a network of everyday
drivers mapping roads as they drive. Hivemapper fleets are critical to
increasing the networkâs ability to capture a wide range of data across
different locations. Fleets enable managers to generate substantially more
rewards than individual contributors while onboarding people who want to
participate but canât purchase a dashcam.

Bradley Meyer was drawn to Hivemapper due to its potential, deploying his own
fleet in the early days of the network as a side hustle. âThereâs a huge
use case for the data thatâs being collected when it comes to street-level
imagery,â he says. He preordered a number of dashcams and started
strategizing about how to build a fleet, which has since grown to 55 drivers
across 15 cities â with even more queued up as he awaits his next shipment.

Now Bradley is helping others build fleets. âThe more that we can deploy
fleets and get people that are on the road a lot during the day, the better it
is for everyone,â he explains. âWe all win with more data collection.â

Here are Bradleyâs strategies for recruiting drivers. ([**See his tips for
operational fleet management here.**](https://hivemapper.com/blog/how-to-
manage-your-hivemapper-fleet-operationally)) Dashcams may be backordered, but
donât wait to start recruiting. âYou might be thinking, âOh my gosh, I
missed the boat.â No â we are still so early in this,â says Bradley.
âAnd even if you have to wait until May, you should have started recruiting
yesterday.â

** _Interested in joining a fleet?_**[**__Register
here__**](https://docs.google.com/forms/d/e/1FAIpQLSeMiVI7kDDZplDBZwWHF75bJb_2xRudFmTPuWwMlzHiJGqy2A/viewform)**
_to be connected with a fleet manager. You can also_**[** __****__join our
Discord and visit #fleet-driver-matching__**](https://discord.gg/w5yzMwpb)**
_._**

## **Develop Your Elevator Pitch**

  * Research Hivemapper thoroughly in order to explain its utility and potential persuasively. Conveying genuine, informed excitement is important.

  * Create a simple, straightforward pitch. Bradley explains that Hivemapper is building the worldâs freshest map using street-level imagery by rewarding drivers for their contributions, unlocking value for businesses that need fresh and accurate map data.

## **Initiate Recruitment Outreach**

  * A frequent traveler and Uber user, Bradley started by directly pitching rideshare drivers. He was also interested in Amazon Flex and DoorDash drivers, who have the capability to mount both front and side dashcams.

  * Target daytime drivers who drive full-time at least 4â5 days a week.

  * Focus on the simplicity and benefits of fleets. Bradley emphasized that his drivers donât pay anything to participate. âItâs an easy sell. I made the capital investment, and I do a token split with the driver.â

  * Speak 20% of the time and listen 80% of the time. This approach helps build rapport and trust with potential recruits.

  * Having a business card helped in getting people to follow through. Bradley found physical cards more effective than virtual ones, serving as a tangible reminder for drivers.

  * Bradley also recommends creating a [_fleet member request form_](https://forms.office.com/pages/responsepage.aspx?id=DQSIkWdsW0yxEjajBLZtrQAAAAAAAAAAAAMAAF5yvkpUNzlKMEFCUTZVVEhFNVFQODNYVThKMU9VNC4u) for drivers looking to join a fleet, and offers his own as a sample.

## **Land Recruits Effectively**

  * Be patient. Not every pitch will be successful, so Bradley encourages aspiring managers to keep pitching drivers.

  * Bradley shares a picture of the dashcam setup to help visualize it. âI also tend to open up the Explorer,â an application that shows Hivemapperâs map coverage across regions. âI find that legitimizes it because itâs almost too good to be true, right? Youâre going to place equipment with someone at no cost to them, and theyâre going to be incentivized for what theyâre already doing every day.â

  * Donât be pushy. The opportunity is already attractive, Bradley notes. After presenting the opportunity, he suggests pausing to let the driver process the info and express interest.

  * Stay in touch with potential recruits. Bradley recommends checking in and providing updates about the project and shipment timelines.

## **Set Token Splits and Expectations**

  * Decide on a fair token split with drivers. Bradley does a 50/50 split, believing itâs fair since âtheyâre the ones that are on the road driving.â He emphasizes fairness and mutual benefit in his approach to managing the fleet.

  * Be transparent about the long-term vision and avoid promising potential rewards to set proper expectations.

  * âOnce the rewards actually hit their wallet, it becomes very real.â

## **Onboard and Train Your Drivers**

  * Ensure that your drivers have the right equipment and understand how to use it effectively. Bradley provides his drivers with the necessary equipment, including the dashcam. He emphasizes the importance of camera setup for optimal data collection.

  * Heâs also adapted his onboarding process for remote setups, including creating tutorials and guides for new drivers to get started.

## **Engage and Monitor Your Fleet**

  * After onboarding new drivers, Bradley monitors their uploads and performance through Explorer to ensure that theyâre mapping effectively.

  * Stay connected with your drivers, invite feedback, and provide regular updates on the project. This is crucial for maintaining a motivated fleet.

## **Implement a Referral Program**

  * Leverage driver relationships to expand the fleet. Referrals from fleet members is a big part of Bradleyâs expansion strategy. 

  * âThe referral program is where Iâve been able to expand the fastest,â notes Bradley. Some 35â40% of the drivers in his fleet were referred from drivers who he originally connected with. âThatâs been the most effective way for me to really expand, and expand quickly.â

  * He offers a 10% override on the rewards of referred drivers.

## **Professionalize Your Fleet**

  * Bradleyâs fleet tips are not financial advice. He advises aspiring fleet managers to consider the legal and administrative aspects seriously, and suggests consulting with an accountant or lawyer about forming an LLC (limited liability company) for your fleet and understanding the implications of 1099 agreements for independent contractors.

* * *

By following Bradleyâs guidance on fleet building and management, you can
start to grow and manage an effective Hivemapper fleet that contributes
quality mapping data to the network.

The insights heâs shared provide a blueprint for recruiting, onboarding, and
retaining motivated drivers over the long term. For more from Bradley, follow
**DePIN Connection** on [_X_](https://twitter.com/DePINConnection),
[_YouTube_](https://www.youtube.com/@DePINConnection), and
[_TikTok_](https://www.tiktok.com/@depin_connection). What are you waiting
for? Start building your fleet today!******

## [Follow us on X (Twitter)](https://twitter.com/Hivemapper) and [join the
community on Discord](https://discord.com/invite/the-official-hivemapper-
community-715439007764316191).

Share Post

Latest Posts

[![](data:image/jpeg;base64,)![Images
Blog Minimake-your-mount-a-4-4-so-we-can-build-better-
maps](https://images.ctfassets.net/n5g2b08mtiib/4CU3O2vh1XkEufK4Tbbw2p/fb577bd1bf174df17e35e22e4e97d522/Hivemapper_-
_Make_Your_Mount_-_Landscape_-
_v1.jpg?w=750&h=375&fl=progressive&q=50&fm=jpg)![Images Blog Minimake-your-
mount-a-4-4-so-we-can-build-better-
maps](https://images.ctfassets.net/n5g2b08mtiib/4CU3O2vh1XkEufK4Tbbw2p/fb577bd1bf174df17e35e22e4e97d522/Hivemapper_-
_Make_Your_Mount_-_Landscape_-
_v1.jpg?w=750&h=375&fl=progressive&q=50&fm=jpg)Make Your Mount a 4/4 So We Can
Build Better Maps!May 13, 2024](/blog/make-your-mount-a-4-4-so-we-can-build-
better-
maps)[![](data:image/png;base64,)![Images
Blog Mininavigating-the-road-ahead-why-your-fleet-needs-bee-hivemappers-
newest](https://images.ctfassets.net/n5g2b08mtiib/4WiP2UuVA2VgslKYIKMcOu/49a6bdb55186b99fe43dc94769ab0692/Hivemapper_-
_Why_Your_Fleets_Needs_Bee_-_Hero_-_v2.2.png?w=750&h=375&q=50&fm=png)![Images
Blog Mininavigating-the-road-ahead-why-your-fleet-needs-bee-hivemappers-
newest](https://images.ctfassets.net/n5g2b08mtiib/4WiP2UuVA2VgslKYIKMcOu/49a6bdb55186b99fe43dc94769ab0692/Hivemapper_-
_Why_Your_Fleets_Needs_Bee_-_Hero_-
_v2.2.png?w=750&h=375&q=50&fm=png)Navigating the Road Ahead: Why Your Fleet
Needs âBeeâ Hivemapperâs Newest DashcamApril 19, 2024](/blog/navigating-
the-road-ahead-why-your-fleet-needs-bee-hivemappers-
newest)[![](data:image/png;base64,)![Images
Blog Mini5-reasons-hivemapper-built-
bee](https://images.ctfassets.net/n5g2b08mtiib/5YoRgz2ke3pD1FOx7mt4kt/f969142f3a5dc4e21b918eafbe9fe95d/Welcome_to_the_Future_of_Mapmaking_-
_v1__1_.png?w=750&h=375&q=50&fm=png)![Images Blog Mini5-reasons-hivemapper-
built-
bee](https://images.ctfassets.net/n5g2b08mtiib/5YoRgz2ke3pD1FOx7mt4kt/f969142f3a5dc4e21b918eafbe9fe95d/Welcome_to_the_Future_of_Mapmaking_-
_v1__1_.png?w=750&h=375&q=50&fm=png)5 Reasons Hivemapper Built the BeeFebruary
21, 2024](/blog/5-reasons-hivemapper-built-bee)

[Read More](https://hivemapper.com/blog/)

[Blog](/blog)

[Careers](/careers)

[Media Kit](/media-kit)

[Privacy](/privacy)

[](https://discord.com/invite/FRWMKyy5v2)[](https://twitter.com/Hivemapper)[](https://www.instagram.com/hivemapper)

