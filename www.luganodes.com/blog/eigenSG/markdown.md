Build the future of Web3 with us. Check out our open positions today!

[![Luganodes
Logo](data:image/svg+xml;base64,)](/)

[Home](/)

Product

Resources

Company

Contact us

[Guides](/blog/?interval=3&category=Guides)

4 min read

# EigenLayer — How to Claim & Stake $EIGEN Tokens with Luganodes

EIGEN Staking Guide

![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='675'%20width='1200'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)

![](data:image/png;base64,)![Cover
Image](/static/22b8ef4af783d1be013027de0af704eb/bb8ee/EIGEN_b3006e7c3b.png)![Cover
Image](/static/22b8ef4af783d1be013027de0af704eb/bb8ee/EIGEN_b3006e7c3b.png)

Published on

15 May, 2024

Copy
Link[](https://twitter.com/intent/tweet?text=Check%20out%20this%20blog%20by%20Luganodes%20-%20%22EigenLayer%20%E2%80%94%20How%20to%20Claim%20%26%20Stake%20%24EIGEN%20Tokens%20with%20Luganodes%22&url=)[](https://www.linkedin.com/sharing/share-
offsite/?url=&title=EigenLayer%20%E2%80%94%20How%20to%20Claim%20%26%20Stake%20%24EIGEN%20Tokens%20with%20Luganodes&summary=Check%20out%20this%20blog%20by%20Luganodes%20-%20%22EigenLayer%20%E2%80%94%20How%20to%20Claim%20%26%20Stake%20%24EIGEN%20Tokens%20with%20Luganodes%22)

# Introduction

Restaking revolutionary, EigenLayer has launched its own token $EIGEN. The
“stake drop” was opened to claimers on May 10th. Eligible users are now able
to claim $EIGEN tokens from
[claims.eigenfoundation.org](https://claims.eigenfoundation.org/). This is the
first phase of the token drops series, and the $EIGEN tokens are currently
non-transferable. As a much-anticipated airdrop, this token will facilitate
the now-famous EigenLayer ecosystem.

# EIGEN Token

In our earlier articles, we have described how [EigenLayer is revolutionising
building projects on blockchain](https://www.luganodes.com/blog/eigenlayer-
explained/). EigenLayer has very efficiently utilised the crypto-economic
security of staked ETH to [power Actively Validated Services
(AVSs)](https://www.luganodes.com/blog/avs-ecosystem-eigenlayer/) and given
rise to the restaking world, another great incentive to earn rewards. In this
quest, the power of decentralised trust has now been unlocked.

However, this trust mechanism reaches its full potential when stakers are also
held responsible for their actions. While ETH provides a good base of
security, the new $EIGEN token will help implement penalties such as slashing
in order to maintain accountability. This creates a more robust environment
for AVSs to function.

As the $EIGEN token soars in popularity and demand, you can learn more about
it in our launch x/Twitter thread.

> EIGEN: The Universal Intersubjective Work Token  
>  
> Universal - Can use it for any application  
>  
> Intersubjective - Enable coordination among external observers. Through its
> intersubjective nature, people can collaborate and identify faults, ensuring
> transparency and trust across the…
> [pic.twitter.com/G8xRdKa9k5](https://t.co/G8xRdKa9k5)
>
> — Yanshu | Luganodes (@0xyanshu) [May 13, 2024](https://twitter.com/0xyanshu/status/1790079293113938000?ref_src=twsrc%5Etfw)

In this guide, we will demonstrate in detail, how to claim $EIGEN tokens and
then stake them using the institutional-grade staking services offered by
Luganodes.

# Validator Details

Validator Name: Luganodes

Operator Address: 0xa3ff1491088715D140bc821A28287cB3f71B43E4

EigenLayer App Link for the Operator:
<https://app.eigenlayer.xyz/operator/0xa3ff1491088715D140bc821A28287cB3f71B43E4>

# Claiming $EIGEN Tokens

## Step-by-Step Claim Guide

### Step 1: Visit the Claims Portal

Access the Eigen Token claims website at the $EIGEN Token Claims Portal.

Click on "ENTER" to proceed.

![e1.png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/e1_403f4c9b95.png)

### Step 2: Connect Your Wallet

Connect your digital wallet by selecting your wallet provider and following
the prompts to establish a secure connection.

![e2.png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/e2_f6cc4088aa.png)

### Step 3: Claim Your Tokens

Follow the on-screen instructions to check your eligibility for the claim.

If eligible, proceed to claim your $EIGEN tokens by following the guided steps
provided on the website.

![e3.png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/e3_d364b72820.png)

# Delegating the Claimed Tokens

## Step-by-Step Delegation Guide

### Step 1: Access the Platform

Navigate to the Eigenlayer staking interface at the [EigenLayer Delegation
Portal](https://app.eigenlayer.xyz/restake/EIGEN).

### Step 2: Deposit Your Tokens

Input the quantity of $EIGEN tokens you wish to deposit in the designated
field.

Note: If your ETH has already been restaked and delegated to an operator, your
$EIGEN will be automatically delegated to the same operator, and no further
action is needed.

![e5.png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/e5_bcec879901.png)

### Step 3: Initiate the Delegation

After a successful deposit, locate and click on the "Delegate" button to
proceed.

![image \(141\).png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/image_141_b9700cf775.png)

### Step 4: Select Your Operator

Utilise the search function to find node operators either by their name or
Ethereum address. Then, select the desired operator's tile to go to their
detailed information page.

![Screenshot 2024-05-15 at 4.11.10 PM.png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/Screenshot_2024_05_15_at_4_11_10_PM_c6246e62ba.png)

### Step 5: Confirm Your Delegation

To delegate all your staked assets to the chosen operator, press the
"Delegate" button on the operator's detail page.

![image \(142\).png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/image_142_0aee034873.png)
![e6.png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/e6_63a104653e.png)

By following these simple steps, you can efficiently delegate your $EIGEN
tokens and start earning rewards with your selected EigenDA node operator.

Apart from $EIGEN staking, you can also restake your ETH with Luganodes.

Maximise your Ethereum investments by restaking with EigenLayer through
Luganodes. Benefit from supporting innovative Actively Validated Services
(AVS) and earn rewards. [Restaking is simple with our detailed guide available
here](https://www.luganodes.com/blog/eigenlayerSG/).

Explore the list of AVSs we currently support, on our [dedicated EigenLayer
page](https://www.luganodes.com/product/eigenlayer/). Learn more about staking
and restaking on our [website](www.luganodes.com), and feel free to [contact
us](https://www.luganodes.com/#getInTouch) for any queries!

## About Luganodes

Luganodes is a world-class, Swiss-operated, non-custodial blockchain
infrastructure provider that has rapidly gained recognition in the industry
for offering institutional-grade services. It was born out of the Lugano Plan
B Program, an initiative driven by Tether and the City of Lugano. Luganodes
maintains an exceptional 99.9% uptime with round-the-clock monitoring by SRE
experts. With support for 40+ PoS networks, it ranks among the top validators
on Polygon, Polkadot, Sui, and Tron. Luganodes prioritizes security and
compliance, holding the distinction of being one of the first staking
providers to adhere to all SOC 2 Type II, GDPR, and ISO 27001 standards as
well as offering Chainproof insurance to institutional clients.

![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='269'%20width='632'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)

![Line
pattern](/static/c1a2eb9200e441157c01110f2f463004/3d614/linepattern3.svg)![Line
pattern](/static/c1a2eb9200e441157c01110f2f463004/3d614/linepattern3.svg)

[![White
logo](data:image/svg+xml;base64,)](/)Secure
staking with leading blockchain infrastructure provider

LN Tech FZE,  
Saif Suite X3-46,  
SAIF Zone, Sharjah, UAE

[![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='288'%20width='462'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)![Verified
badge](/static/6ba07c96436f2c9565f80d24555d7bac/edf67/vsp-pro.svg)![Verified
badge](/static/6ba07c96436f2c9565f80d24555d7bac/edf67/vsp-
pro.svg)](https://www.stakingrewards.com/savings/luganodes/)

Products[Staking](/product/staking/)

[EigenLayer Staking](/product/eigenlayer/)

[zkRaaS](/product/zkraas/)

[Staking API](/product/stakingapi/)

[Managed Nodes](/product/managednodes/)

[Terminal](/product/luganodesterminal/)

ResourceClient Types

[Custodian](/client/custodian/)[Exchanges](/client/exchanges/)[Investors
(VCs)](/client/investors/)

Case Studies

[CoinList](/case-study/Coinlist_CS/)[Bitfinex](/case-study/Bitfinex_CS/)

[Blog](/blog/)

[Staking Guides](/blog/?interval=3&category=Guides#blog-list)

[Newsletter](/#signup)

[Docs](https://docs.luganodes.com/)

Company[About us](/aboutus/)

[Get in Touch](/#getInTouch)

[Security](https://security.luganodes.com/)

Social[Twitter](https://twitter.com/luganodes)

[LinkedIn](https://www.linkedin.com/company/luganodes/)

[Telegram](https://t.me/luganodesofficial/)

[Medium](https://medium.com/luganodes)

[YouTube](https://www.youtube.com/@luganodes)

[Terms of use](/termsofservice/)[Privacy Policy](/privacypolicy/)

© 2024 Luganodes | All rights reserved

[Terms of use](/termsofservice/)[Privacy Policy](/privacypolicy/)

