Build the future of Web3 with us. Check out our open positions today!

[![Luganodes
Logo](data:image/svg+xml;base64,)](/)

[Home](/)

Product

Resources

Company

Contact us

[Network](/blog/?interval=3&category=Network)

6 min read

# An Overview: The EVMOS Ecosystem

Fast Forward to ETH's future?

![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='675.3623188405797'%20width='1200'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)

![](data:image/webp;base64,UklGRmoAAABXRUJQVlA4IF4AAACQAwCdASoUAAsAPtFWpEuoJKOhsAgBABoJZwAAW64D6MRAMqgAAP70pvdHddQW6aaWJRjnBeN6pg0WbKV3028GXaQpkksO7VJ6tE4r+Lt9hRqF/cH1CFQc1atsQAAA)![Cover
Image](/static/cce1431751b2f72013b26049d658b5ce/88eb6/1_e83baa7772.webp)![Cover
Image](/static/cce1431751b2f72013b26049d658b5ce/88eb6/1_e83baa7772.webp)

Published on

8 Oct, 2022

Copy
Link[](https://twitter.com/intent/tweet?text=Check%20out%20this%20blog%20by%20Luganodes%20-%20%22An%20Overview%3A%20The%20EVMOS%20Ecosystem%22&url=)[](https://www.linkedin.com/sharing/share-
offsite/?url=&title=An%20Overview%3A%20The%20EVMOS%20Ecosystem&summary=Check%20out%20this%20blog%20by%20Luganodes%20-%20%22An%20Overview%3A%20The%20EVMOS%20Ecosystem%22)

# Salient Features:

  * No of Validators: 150
  * Validation Type: Proof-of-Stake
  * Coin: EVMOS
  * Distribution of network fees: 50/50 split between developers and validator
  * Offers full Web3 and EVM compatibility
  * Enables high throughput via Tendermint Core
  * Horizontally scalable via IBC
  * Offers fast transaction finality
  * Popular projects in the ecosystem: 4T2, coinGecko, Ankr, Asteroids Money, tofuNet, Orbital Apes
  * Total revenue (last 30 days): $3,814
  * Twitter Followers: 110,679
  * Discord Followers: 36,011

![2.webp](https://better-
triumph-f08c8a30a5.media.strapiapp.com/2_df5181b41f.webp) Cosmos ecosystem —
top five contenders  
Source: coingecko.com

# Introduction:

The “**new frontier** ” in the blockchain space is what Evmos claims itself to
be. Evmos, EVM on cosmos, is an application-agnostic blockchain network that
leverages the Cosmos SDK framework to realize the Ethereum virtual machine
(EVM), hence accomplishing full compatibility with Ethereum. Evmos envisions
bringing the now disconnected worlds of Ethereum-based applications and assets
to the interoperable networks of the Cosmos ecosystem under one umbrella, all
while catering toward developer and user incentives.

The Tharsis team broke ground on the Ethermint project (now Evmos) in 2016
with an aim to **establish scalable Ethereum or EVM-based applications built
on top of the Tendermint consensus protocol**. The project has undergone
drastic changes since its inception, apart from its name (Evmos does sound
cooler!), and now boasts of a very different ecosystem.

**What is Evmos?**

Ethereum Virtual Machine (EVM) on Cosmos — Evmos is a scalable, high-
throughput blockchain backed by the Proof of Stake (PoS) consensus mechanism.
This network manages to retain its identity, and security, while also being
fully interoperable and compatible with Ethereum.

**Interoperability!**

Evmos is Inter Blockchain Communication (IBC) enabled, and EVM based which
enables it to materialize the composability of Ethereum and the fast finality
of Tendermint into a giant web of interoperable blockchain ecosystems. So,
developers can now run and deploy Ethereum smart contracts on app-specific
Cosmos networks by harnessing the versatility of Evmos.

#

With an end goal to create a scalable layer-1 blockchain, Evmos employs the
use of Cosmos SDK, the Application Blockchain Interface (ABCI), and the
Ethereum virtual machine. The Cosmos SDK is the blockchain infrastructure
provided by Cosmos for creating blockchains with PoS or PoA (Proof-of-
Authority). This SDK enables connections to other Cosmos-based blockchains
from the beginning, hence giving dApps access to an extended user base. This
is made possible through the Cosmos’ inter-blockchain communication (IBC).
Rightfully so, **Cosmos is called the Internet of Blockchains**. ABCI is the
interface between Tendermint and the dApp running. ABCI is the infrastructure
that allows Evmos to replace Ethereum proof-of-work with Cosmos Proof-of-
Stake. Since every Ethereum node runs on EVM, an application created on EVM is
capable of running on all Ethereum nodes.

**Tech stack:**

The Evmos network is developed using the Cosmos Development Kit (SDK) on top
of the Tendermint Core Byzantine Fault Tolerance consensus mechanism.

# Tokenomics of Evmos:

The Evmos blockchain presents its users with a native blockchain token of the
same name (EVMOS) which serves the many functions Evmos is capable of
performing. Usage as management tokens, gas fees, and as incentives to endorse
the use applications built on Evmos are among some common use cases. It can
also be used to provide rewards to developers and network operators for their
services through the built-in revenue-sharing model called the Evmos dApp
store.

Among the numerous things that Evmos does differently, tokenomics is one of
the most interesting. In stark contrast with several other blockchain tokens,
the Evmos team did not conduct any presale or pre-mining activities for its
token distribution. Instead, an offering of 200 million was proposed as the
initial genesis, soaring up to a total offering of 1 billion. The project
conducted a “**Rektdrop** ” to distribute the chain’s native token. This
genesis airdrop awarded the EVMOS token to a wide array of users, ranging from
longtime project supporters and Ethereum power users, to the victims of hacks
and exploits on Ethereum. Consequently, these tokens were majorly distributed
among users of Ethereum’s most popular applications, such as Aave, Olympus, as
well as users of the Cosmos ecosystem.

The initial genesis offering confers to the following distribution:

  * **Staking rewards — 40%**
  * **Team — 25%**
  * **Usage rewards — 25%**
  * **Pool — 10%**

A more detailed view of this distribution is illustrated in the embedded
figure.

![3.webp](https://better-
triumph-f08c8a30a5.media.strapiapp.com/3_1308251d59.webp) Source:
coingecko.com

This POS-based blockchain is secured by validators and delegators. At the time
of writing this document, the active validator set of Evmos comprises of 150
validators and 40% of the newly issued tokens are used to reward the active
validators and their delegates. The amount of rewards is commensurate to the
number of staked tokens.

# Final Takeaways:

Evmos is a pioneer in the establishment of cross-chain connectivity between
the seemingly disparate worlds of Ethereum and Cosmos. By enabling
interoperability between these chains, Evmos presents an attractive bargain to
the users and developers of the Ethereum network, who are highly plagued by
transaction costs and network congestion. Not to mention, it opens a plethora
of opportunities for Cosmos developers to expand their reach to Ethereum, the
industry-leading smart contract network. The Evmos project is led by a very
competent team spearheaded by co-founders Federico Kunze Kullmer, a full-time
Cosmos contributor since 2017, and Akash Khosla, a software engineer with 5
years of experience. The co-founders harbor a shared view of revolutionizing
the blockchain community, and with Evmos they’re headed in the right
direction. The Evmos community is expanding rapidly, and with the upcoming
rollout of the Evmos EVM bridge, Evmos will allow converting tokens that don’t
require to be initially run on Cosmos.

Evmos is also garnering tremendous support from their communities on Twitter
and Discord, where the follower count is only surging.

![4.webp](https://better-
triumph-f08c8a30a5.media.strapiapp.com/4_e27b82fab0.webp) Twitter follower
statistics of [@EvmosOrg](https://twitter.com/EvmosOrg) (Aug, 22 — Sept, 22)
Source: alpha growth.com

The upcoming release of a public roadmap is set to unveil what more Evmos has
in store for its users and developers. Quoting the Evmos enthusiasts on the
potential of Evmos — “**Evmos is basically ETH 3.0 before ETH 2.0 is ready!**
”.

## About Luganodes

Luganodes is a world-class, Swiss-operated, non-custodial blockchain
infrastructure provider that has rapidly gained recognition in the industry
for offering institutional-grade services. It was born out of the Lugano Plan
B Program, an initiative driven by Tether and the City of Lugano. Luganodes
maintains an exceptional 99.9% uptime with round-the-clock monitoring by SRE
experts. With support for 40+ PoS networks, it ranks among the top validators
on Polygon, Polkadot, Sui, and Tron. Luganodes prioritizes security and
compliance, holding the distinction of being one of the first staking
providers to adhere to all SOC 2 Type II, GDPR, and ISO 27001 standards as
well as offering Chainproof insurance to institutional clients.

![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='269'%20width='632'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)

![Line
pattern](/static/c1a2eb9200e441157c01110f2f463004/3d614/linepattern3.svg)![Line
pattern](/static/c1a2eb9200e441157c01110f2f463004/3d614/linepattern3.svg)

[![White
logo](data:image/svg+xml;base64,)](/)Secure
staking with leading blockchain infrastructure provider

LN Tech FZE,  
Saif Suite X3-46,  
SAIF Zone, Sharjah, UAE

[![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='288'%20width='462'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)![Verified
badge](/static/6ba07c96436f2c9565f80d24555d7bac/edf67/vsp-pro.svg)![Verified
badge](/static/6ba07c96436f2c9565f80d24555d7bac/edf67/vsp-
pro.svg)](https://www.stakingrewards.com/savings/luganodes/)

Products[Staking](/product/staking/)

[EigenLayer Staking](/product/eigenlayer/)

[zkRaaS](/product/zkraas/)

[Staking API](/product/stakingapi/)

[Managed Nodes](/product/managednodes/)

[Terminal](/product/luganodesterminal/)

ResourceClient Types

[Custodian](/client/custodian/)[Exchanges](/client/exchanges/)[Investors
(VCs)](/client/investors/)

Case Studies

[CoinList](/case-study/Coinlist_CS/)[Bitfinex](/case-study/Bitfinex_CS/)

[Blog](/blog/)

[Staking Guides](/blog/?interval=3&category=Guides#blog-list)

[Newsletter](/#signup)

[Docs](https://docs.luganodes.com/)

Company[About us](/aboutus/)

[Get in Touch](/#getInTouch)

[Security](https://security.luganodes.com/)

Social[Twitter](https://twitter.com/luganodes)

[LinkedIn](https://www.linkedin.com/company/luganodes/)

[Telegram](https://t.me/luganodesofficial/)

[Medium](https://medium.com/luganodes)

[YouTube](https://www.youtube.com/@luganodes)

[Terms of use](/termsofservice/)[Privacy Policy](/privacypolicy/)

© 2024 Luganodes | All rights reserved

[Terms of use](/termsofservice/)[Privacy Policy](/privacypolicy/)

