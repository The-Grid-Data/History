Build the future of Web3 with us. Check out our open positions today!

[![Luganodes
Logo](data:image/svg+xml;base64,)](/)

[Home](/)

Product

Resources

Company

Contact us

[Update](/blog/?interval=3&category=Update)

4 min read

# Luganodes BlockBytes— June Edition 1

Network Updates!

![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='675.3623188405797'%20width='1200'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)

![](data:image/webp;base64,)![Cover
Image](/static/150089a3aad2bdd5bca5fc08d76f394f/88eb6/1_a_SSXC_Ycy_Es3ica79_Ti_XP_3_A_9e994b79fb.webp)![Cover
Image](/static/150089a3aad2bdd5bca5fc08d76f394f/88eb6/1_a_SSXC_Ycy_Es3ica79_Ti_XP_3_A_9e994b79fb.webp)

Published on

20 June, 2023

Copy
Link[](https://twitter.com/intent/tweet?text=Check%20out%20this%20blog%20by%20Luganodes%20-%20%22Luganodes%20BlockBytes%E2%80%94%20June%20Edition%201%22&url=)[](https://www.linkedin.com/sharing/share-
offsite/?url=&title=Luganodes%20BlockBytes%E2%80%94%20June%20Edition%201&summary=Check%20out%20this%20blog%20by%20Luganodes%20-%20%22Luganodes%20BlockBytes%E2%80%94%20June%20Edition%201%22)

Hello Staker. Welcome to Luganodes BlockBytes, a brand-new series to bring you
the latest network updates and major industry breakthroughs.

# Polygon 2.0 Ultimate Value Layer

![2_U3fUAwuKkodkYcc-e_PiaQ.webp](https://better-
triumph-f08c8a30a5.media.strapiapp.com/2_U3f_U_Awu_Kkodk_Ycc_e_Pia_Q_a0e9e68cb7.webp)
The Polygon team has unveiled Polygon 2.0, the blueprint for building the
Value Layer of the Internet. Polygon 2.0 brings radical upgrades to every
aspect of Polygon, from protocol architecture to tokenomics to governance. It
aims to unlock unlimited scalability and unified liquidity through ZK-powered
L2 chains and a novel cross-chain coordination protocol. The Polygon 2.0
roadmap is set to be unveiled gradually, covering essential topics such as the
future of the PoS chain, token utility, and the transition to greater
community governance.

With formal governance processes in place, the Polygon ecosystem empowers
builders, creators, and stakeholders to shape the network’s future. As one of
the leading validators on the network, Luganodes is thrilled to express our
support and enthusiasm for this groundbreaking upgrade. We will be actively
engaging in the Polygon community, participating in AMAs, and providing
support to help Polygon realise its vision of becoming the ultimate Value
Layer.

#

![3_qpWZowJ-fNlF2PAH-PYyyw.webp](https://better-
triumph-f08c8a30a5.media.strapiapp.com/3_qp_W_Zow_J_f_Nl_F2_PAH_P_Yyyw_142b504e32.webp)

  * **Highlight** : NEAR continues to thrive with a robust and scalable infrastructure, with daily transactions averaging 393K in Q1 2023, and daily active accounts hitting an all-time high of 64K.
  * **TVL** : NEAR’s ecosystem is thriving with a TVL of $65M in Q1 2023. Despite a slight decline in Q1, the DeFi sector remains promising. Protocols like Bastion Protocol, [](https://twitter.com/finance_ref) Ref Finance, Burrow, and Orderly Network continue to lead the way, while the rest contribute to the growing TVL.
  * **Developer Communities** : In 2022, NEAR experienced a remarkable 40% increase in developers and a staggering 20x growth since 2018. Initiatives like the NEAR Protocol Primer, partnerships with NYU SPS, and Startup Wise Guys demonstrate NEAR’s dedication to developer engagement and education.
  * **Decentralisation** : The launch of Phase 1 of Nightshade sharding tripled the network’s validator capacity, surpassing 200 active validator nodes; and total staked NEAR has reached $525M, strengthening the network’s security and decentralisation.
  * **Capital Allocation** : NEAR is actively working towards decentralising capital allocation. Community DAOs, including Marketing DAO, NEAR DevHub, Creatives DAO, and NEAR Digital Collective, empower the community to make funding decisions.

#

![4_j9O9aKHywJsiPWWE2ZO0CA.webp](https://better-
triumph-f08c8a30a5.media.strapiapp.com/4_j9_O9a_K_Hyw_Jsi_PWWE_2_ZO_0_CA_7f25bcec45.webp)

  * **Highlight** : The robustness of the Solana network is evident in its continuous growth and resilience. With over 300K unique stakers and more than 3400 validators, the network has shown remarkable decentralisation and belief from the community.
  * **Network Upgrade** : Solana’s performance has been enhanced through network upgrades such as QUIC, stake-weighted QoS, and local fee markets. These improvements have optimised network performance, enabling high-volume events like the recent Madlads xNFT mint to occur smoothly with minimal issues.
  * **Operational Nakamoto Coefficients** : It places Solana at the forefront, with a score of 1.9. This further emphasises its robustness and ability to handle increased network demands.
  * **DEX** : The rise of gateway interactions from gaming applications and the emergence of new consumer platforms reflect changing user preferences and expanding use cases beyond DEXs and NFT markets.
  * **Developer** : Developers on Solana now have access to Iron forge, a powerful tool that simplifies development and abstracts complexities. Developers can focus on creating innovative dapps instead of spending time on backend integrations. It’s a gateway to serverless Solana development.
  * **NFT** : Solana’s commitment to the NFT space is evident with the Solana NFT Showdown, a unique pitch competition fostering the next generation of NFT brands, aiming to bring real-world products to verticals like fashion, entertainment, and gaming.

#

![5_9P_S3V3op1gpc_-Trxv9Wg.webp](https://better-
triumph-f08c8a30a5.media.strapiapp.com/5_9_P_S3_V3op1gpc_Trxv9_Wg_1a4a6be4b0.webp)

  * **Highlight** : In the week of 5th June, TVL on the Ronin Network has crossed the $80M mark, indicating a healthy and thriving ecosystem.
  * **NFT** : Ronin Network has now become the second blockchain by all-time NFT volume, trailing only Ethereum.
  * **Active Addresses** : The number of active addresses on Ronin has steadily increased. In May, we witnessed a growth of approximately 3,000 addresses.
  * **Binance** : Binance will continue to support the Ronin network upgrade and hard fork. They have integrated native AXS, SLP and USDC withdrawals and deposits.

## About Luganodes

Luganodes is a world-class, Swiss-operated, non-custodial blockchain
infrastructure provider that has rapidly gained recognition in the industry
for offering institutional-grade services. It was born out of the Lugano Plan
B Program, an initiative driven by Tether and the City of Lugano. Luganodes
maintains an exceptional 99.9% uptime with round-the-clock monitoring by SRE
experts. With support for 40+ PoS networks, it ranks among the top validators
on Polygon, Polkadot, Sui, and Tron. Luganodes prioritizes security and
compliance, holding the distinction of being one of the first staking
providers to adhere to all SOC 2 Type II, GDPR, and ISO 27001 standards as
well as offering Chainproof insurance to institutional clients.

![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='269'%20width='632'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)

![Line
pattern](/static/c1a2eb9200e441157c01110f2f463004/3d614/linepattern3.svg)![Line
pattern](/static/c1a2eb9200e441157c01110f2f463004/3d614/linepattern3.svg)

[![White
logo](data:image/svg+xml;base64,)](/)Secure
staking with leading blockchain infrastructure provider

LN Tech FZE,  
Saif Suite X3-46,  
SAIF Zone, Sharjah, UAE

[![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='288'%20width='462'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)![Verified
badge](/static/6ba07c96436f2c9565f80d24555d7bac/edf67/vsp-pro.svg)![Verified
badge](/static/6ba07c96436f2c9565f80d24555d7bac/edf67/vsp-
pro.svg)](https://www.stakingrewards.com/savings/luganodes/)

Products[Staking](/product/staking/)

[EigenLayer Staking](/product/eigenlayer/)

[zkRaaS](/product/zkraas/)

[Staking API](/product/stakingapi/)

[Managed Nodes](/product/managednodes/)

[Terminal](/product/luganodesterminal/)

ResourceClient Types

[Custodian](/client/custodian/)[Exchanges](/client/exchanges/)[Investors
(VCs)](/client/investors/)

Case Studies

[CoinList](/case-study/Coinlist_CS/)[Bitfinex](/case-study/Bitfinex_CS/)

[Blog](/blog/)

[Staking Guides](/blog/?interval=3&category=Guides#blog-list)

[Newsletter](/#signup)

[Docs](https://docs.luganodes.com/)

Company[About us](/aboutus/)

[Get in Touch](/#getInTouch)

[Security](https://security.luganodes.com/)

Social[Twitter](https://twitter.com/luganodes)

[LinkedIn](https://www.linkedin.com/company/luganodes/)

[Telegram](https://t.me/luganodesofficial/)

[Medium](https://medium.com/luganodes)

[YouTube](https://www.youtube.com/@luganodes)

[Terms of use](/termsofservice/)[Privacy Policy](/privacypolicy/)

© 2024 Luganodes | All rights reserved

[Terms of use](/termsofservice/)[Privacy Policy](/privacypolicy/)

