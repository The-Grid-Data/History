Build the future of Web3 with us. Check out our open positions today!

[![Luganodes
Logo](data:image/svg+xml;base64,)](/)

[Home](/)

Product

Resources

Company

Contact us

[Guides](/blog/?interval=3&category=Guides)

3 min read

# EigenLayer — How to Restake with Luganodes

EigenLayer Restaking Guide

![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='675'%20width='1200'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)

![](data:image/png;base64,)![Cover
Image](/static/47827df4dd1f0c1c28bf6f24443010e7/bb8ee/restaking_74b8a47fff.png)![Cover
Image](/static/47827df4dd1f0c1c28bf6f24443010e7/bb8ee/restaking_74b8a47fff.png)

Published on

15 Apr, 2024

Copy
Link[](https://twitter.com/intent/tweet?text=Check%20out%20this%20blog%20by%20Luganodes%20-%20%22EigenLayer%20%E2%80%94%20How%20to%20Restake%20with%20Luganodes%22&url=https%3A%2F%2Fwww.luganodes.com%2Fblog%2FeigenlayerSG%2F)[](https://www.linkedin.com/sharing/share-
offsite/?url=https%3A%2F%2Fwww.luganodes.com%2Fblog%2FeigenlayerSG%2F&title=EigenLayer%20%E2%80%94%20How%20to%20Restake%20with%20Luganodes&summary=Check%20out%20this%20blog%20by%20Luganodes%20-%20%22EigenLayer%20%E2%80%94%20How%20to%20Restake%20with%20Luganodes%22)

# Introduction:

EigenLayer is a restaking protocol on Ethereum, enabling users to restake
native ETH or LSTs for projects beyond Ethereum. It extends Ethereum's
security to diverse protocols/services, granting developers access to its
massive staked capital base and validator set.

# Validator Details:

Validator Name: Luganodes

Operator Address: 0xa3ff1491088715D140bc821A28287cB3f71B43E4

EigenLayer App Link for the Operator:
<https://app.eigenlayer.xyz/operator/0xa3ff1491088715D140bc821A28287cB3f71B43E4>

# Unbonding Period:

  * Unbonding period is a timeframe in which the funds are locked when you unstake

  * The unbonding period on EigenLayer for Restaked ETH is roughly 7 days as of right now, after which you can finalize the withdrawal, and the undelegated amount will be transferred to your available balance.

# Step-by-Step Staking Guide

## Step 1 - Liquid Staking your ETH for LST

Head over to your preferred liquid staking platform, In this guide we’re using
<https://stake.lido.fi/>

Connect using the wallet in which you’re holding ETH

![1.png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/1_ff96f61128.png)

Once connected, enter the amount of ETH you want to stake to get liquid staked
Ether (Please make sure you have enough ETH left to cover for gas in the next
steps)

![3.png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/3_053d3a9e1a.png)

Confirm the transaction

![5.png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/5_3e1aa46a80.png)

Wait for confirmation

![6.png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/6_92fbc524e1.png)

Once confirmed, you’ll see stETH in your wallet,(If you don’t see stETH, then
you can add manually on MetaMask using “import token”)

![7.png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/7_257c553f6d.png)

Now, you have successfully staked your ETH and now have and LST (Liquid Staked
Token). In the next steps we will restake this LST into the EigenLayer app

## Step 2 - Restaking your LST into EigenLayer

To restake your LST into EigenLayer, head over to the official EigenLayer app
(<https://app.eigenlayer.xyz/> )

Now connect your wallet you used while Liquid Staking in the Step 1

![1 \(1\).png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/1_1_612479bcbb.png)

Sign the transaction to confirm your wallet

![3 \(1\).png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/3_1_5d2dfcd68f.png)

Select the LST you want to restake, Since we liquid staked our ETH with Lido,
we chose Lido staked ether

![4.png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/4_c57f9d910d.png)

Now we enter the amount of stETH we want to restake and hit deposit

![6 \(1\).png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/6_1_d90a396412.png)

It will ask you to confirm spending cap request

![7 \(1\).png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/7_1_700bc126d8.png)

Now hit the deposit again and confirm the trx

![8.png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/8_72c25c0ab3.png)

Once confirmed, you can check you restaked ETH on your Dashboard

![10.png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/10_8f9aaaee60.png)

In the next step we will see how to delegate your restaked ETH to an operator

## Step 3 - Delegating your restaked LST to an Operator

Head over to the official EigenLayer page and connect using the same wallet
you used in Step 1 and 2

Then click on “Delegate your Stake”

![1 \(2\).png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/1_2_e015182ec6.png)

Under the list of operators, look for your preferred operator.

Luganodes is a leading Swiss-operated Institutional-Grade blockchain
infrastructure provider, managing over $3 Billion in staked assets across 45+
networks.

We run ~7%(second largest) of the Natively Restaked Ethereum on bare metal
infrastructure with 99.99% uptime and will distribute airdrop to delegators.

![2.png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/2_deb959539c.png)

Click on Delegate

![3 \(2\).png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/3_2_66e68a7a83.png)

Confirm the trx

![4 \(1\).png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/4_1_2d4ed8aacf.png)

Once confirmed, you can check your restaked amount, delegated amount and you
operator under the Dashboard

![6 \(2\).png](https://better-
triumph-f08c8a30a5.media.strapiapp.com/6_2_a313fdbffa.png)

## About Luganodes

Luganodes is a world-class, Swiss-operated, non-custodial blockchain
infrastructure provider that has rapidly gained recognition in the industry
for offering institutional-grade services. It was born out of the Lugano Plan
B Program, an initiative driven by Tether and the City of Lugano. Luganodes
maintains an exceptional 99.9% uptime with round-the-clock monitoring by SRE
experts. With support for 40+ PoS networks, it ranks among the top validators
on Polygon, Polkadot, Sui, and Tron. Luganodes prioritizes security and
compliance, holding the distinction of being one of the first staking
providers to adhere to all SOC 2 Type II, GDPR, and ISO 27001 standards as
well as offering Chainproof insurance to institutional clients.

Staking Guides

[![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='281'%20width='500'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)![](data:image/png;base64,)![Blog
thumbnail](/static/22b8ef4af783d1be013027de0af704eb/86bab/EIGEN_b3006e7c3b.png)![Blog
thumbnail](/static/22b8ef4af783d1be013027de0af704eb/86bab/EIGEN_b3006e7c3b.png)[Guides](/blog/?interval=3&category=Guides)EigenLayer
— How to Claim & Stake $EIGEN Tokens with LuganodesThis guide outlines the
procedures for claiming and staking $EIGEN tokens with Luganodes, following
EigenLayer's new token launch. It provides step-by-step instructions for users
to participate in the staking process effectively.15 May, 20244 min
read](/blog/eigenSG/)[![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='281'%20width='500'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)![](data:image/png;base64,)![Blog
thumbnail](/static/9a20e544cf2d9d7882987abf3b31fede/86bab/stacking_9d77bef22c.png)![Blog
thumbnail](/static/9a20e544cf2d9d7882987abf3b31fede/86bab/stacking_9d77bef22c.png)[Guides](/blog/?interval=3&category=Guides)
Stacks — How to Stack $STX with LuganodesExplore the benefits of Stacking on
Stacks L2 with this guide, detailing how to securely stack STX tokens using
Luganodes, and maximize returns on the Stacks blockchain. Perfect for
enhancing your crypto investments.5 May, 20244 min read](/blog/StacksSG/)

[View all posts](/blog)

Subscribe to get our latest updates!

New products & networks supported, staking research, blog updates, and
exclusive offers directly in your inbox.

Email

Staker Type

Type of Staker

By subscribing, I accept the [privacy policy](/privacypolicy).Subscribe

## Suggested  
articles

### Stay up to date

[Read All](/blog/?category=Guides)

[![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='844'%20width='1500'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)![](data:image/png;base64,)![Blog
image](/static/ca3a61912a5670dc933267f568f44ed3/64737/Staking_Guide_aptos_7f0a29eb11.png)![Blog
image](/static/ca3a61912a5670dc933267f568f44ed3/64737/Staking_Guide_aptos_7f0a29eb11.png)GuidesAptos
— How to Stake $APT Tokens with LuganodesAptos is a layer 1 blockchain built
with the Move programming language. It promises users increased scalability,
reliability, security, and usability, with low transaction costs. In this
guide, we’ll show you how to delegate your $APT tokens with Luganodes and
start earning rewards in a few simple
steps.](/blog/APTSG)[![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='844'%20width='1500'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)![](data:image/png;base64,)![Blog
image](/static/8136906e65429584ed05b230a1bdb045/64737/Staking_Guide_avalance_fcf7c83454.png)![Blog
image](/static/8136906e65429584ed05b230a1bdb045/64737/Staking_Guide_avalance_fcf7c83454.png)GuidesAvalanche
— How to Stake $AVAX Tokens with LuganodesAvalanche is an open-sourced and
smart contracts compatible platform created to develop decentralized
applications (dApps) and enterprise blockchain protocols. In this guide, we’ll
show you how to delegate your $AVAX tokens with Luganodes and start earning
rewards in a few simple
steps.](/blog/AVAXSG)[![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='843'%20width='1500'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)![](data:image/png;base64,)![Blog
image](/static/7d6622aeb2275598efe7f5add6b4b8e2/e30c5/Staking_Guide_sui_a015bf2ac0.png)![Blog
image](/static/7d6622aeb2275598efe7f5add6b4b8e2/e30c5/Staking_Guide_sui_a015bf2ac0.png)GuidesSUI
— How to Stake $SUI Tokens with LuganodesSui is a scalable smart contract
platform with low-latency and asset-centric adaptation written in Move. In
this guide, we’ll show you how to delegate your $SUI tokens with Luganodes and
start earning rewards in a few simple
steps.](/blog/SUISG)[![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='844'%20width='1500'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)![](data:image/png;base64,)![Blog
image](/static/2d1b276cefa8f950b4e309b2c3f34041/64737/Staking_Guide_multiversx_d6c5720794.png)![Blog
image](/static/2d1b276cefa8f950b4e309b2c3f34041/64737/Staking_Guide_multiversx_d6c5720794.png)GuidesMultiversX
— How to Stake $EGLD Tokens with LuganodesHow to Stake $EGLD Tokens with
Luganodes](/blog/EGLDSG)[![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='844'%20width='1500'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)![](data:image/png;base64,)![Blog
image](/static/758885d82fbadc31541e62e65ff7740e/64737/image_124_87c5963afc.png)![Blog
image](/static/758885d82fbadc31541e62e65ff7740e/64737/image_124_87c5963afc.png)GuidesbitsCrunch
— How to Stake $BCUT Tokens with LuganodesDive into the future of blockchain
analytics by staking your $BCUT tokens with Luganodes, leveraging BitsCrunch's
AI-powered insights for rewards. Follow our simple guide to contribute to
network security and earn in just a few steps.](/blog/BCUTSG)

![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='269'%20width='632'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)

![Line
pattern](/static/c1a2eb9200e441157c01110f2f463004/3d614/linepattern3.svg)![Line
pattern](/static/c1a2eb9200e441157c01110f2f463004/3d614/linepattern3.svg)

[![White
logo](data:image/svg+xml;base64,)](/)Secure
staking with leading blockchain infrastructure provider

LN Tech FZE,  
Saif Suite X3-46,  
SAIF Zone, Sharjah, UAE

[![](data:image/svg+xml;charset=utf-8,%3Csvg%20height='288'%20width='462'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E)![Verified
badge](/static/6ba07c96436f2c9565f80d24555d7bac/edf67/vsp-pro.svg)![Verified
badge](/static/6ba07c96436f2c9565f80d24555d7bac/edf67/vsp-
pro.svg)](https://www.stakingrewards.com/savings/luganodes/)

Products[Staking](/product/staking/)

[EigenLayer Staking](/product/eigenlayer/)

[zkRaaS](/product/zkraas/)

[Staking API](/product/stakingapi/)

[Managed Nodes](/product/managednodes/)

[Terminal](/product/luganodesterminal/)

ResourceClient Types

[Custodian](/client/custodian/)[Exchanges](/client/exchanges/)[Investors
(VCs)](/client/investors/)

Case Studies

[CoinList](/case-study/Coinlist_CS/)[Bitfinex](/case-study/Bitfinex_CS/)

[Blog](/blog/)

[Staking Guides](/blog/?interval=3&category=Guides#blog-list)

[Newsletter](/#signup)

[Docs](https://docs.luganodes.com/)

Company[About us](/aboutus/)

[Get in Touch](/#getInTouch)

[Security](https://security.luganodes.com/)

Social[Twitter](https://twitter.com/luganodes)

[LinkedIn](https://www.linkedin.com/company/luganodes/)

[Telegram](https://t.me/luganodesofficial/)

[Medium](https://medium.com/luganodes)

[YouTube](https://www.youtube.com/@luganodes)

[Terms of use](/termsofservice/)[Privacy Policy](/privacypolicy/)

© 2024 Luganodes | All rights reserved

[Terms of use](/termsofservice/)[Privacy Policy](/privacypolicy/)

