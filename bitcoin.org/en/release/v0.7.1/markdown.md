...
by Matt Corallo to automatically keep up-to-date. Just type: sudo apt-add-
...
Explanation of -detachdb (and the new âstop trueâ RPC command): The
Berkeley DB database library stores data in both â.datâ and âlogâ
files, so the database is always in a consistent state, even in case of power
failure or other sudden shutdown. The format of the â.datâ files is
portable between different versions of Berkeley DB, but the âlogâ files
are notâ even minor version differences may have incompatible âlogâ
files. The -detachdb option moves any pending changes from the âlogâ files
to the âblkindex.datâ file for maximum compatibility, but makes shutdown
much slower. Note that the âwallet.datâ file is always detached, and
versions prior to 0.6.0 detached all databases at shutdown.
