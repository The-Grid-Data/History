...
Technical note: the encrypted walletâs âkeypoolâ will be regenerated the
...
Bitcoin-Qt: Address Book, then New Addressâ¦
...
run the `getnewaddress` RPC command.
  ...
  4. Shut down Bitcoin, then backup the wallet.dat file. IMPORTANT: be sure to request a new bitcoin address before backing up, so that the âkeypoolâ is regenerated and backed up.

âSecurity in depthâ is always a good idea, so choosing a secure location
...
## MAJOR GUI CHANGES
  ...
  * âSplashâ graphics at startup that show address/wallet/blockchain loading progress.

  * âSynchronizing with networkâ progress bar to show block-chain download progress.
